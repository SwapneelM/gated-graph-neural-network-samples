## Instructions to run on CMG GPU
The code requires you to work with Tensorflow 1.3.0 so we install the GPU version
Tensorflow GPU 1.3 seems to require [Cuda 8.0](https://anaconda.org/anaconda/cudatoolkit) with libcudnn.so.6 which is not available unless you register with the [Nvidia Developer Program](https://developer.nvidia.com/rdp/cudnn-archive)

```
conda create -n ggnn-env python==3.5
conda activate ggnn-env

conda install -c rdkit rdkit

pip install -r cmg-gpu-requirements.txt
```

Sign up for the Nvidia Developer Program and obtain the libcudnn.so.6 archive. Extract the archive and you should have a folder ``cuda`` with two sub-folders.

Now copy/transfer the downloaded libcudnn.so.6 files (once you extract the tar.gz archive, inside the ``cuda/lib64`` folder there are 3 files to be copied - ``libcudnn.so``; ``libcudnn.so.6``; ``libcudnn.so.6.0.21`` totaling ~300MB) into the ``$CONDA_PREFIX/lib`` folder so that it is available on the ``$PATH`` and you should be able to run the GGNN training on the CMG GPU 1080.
